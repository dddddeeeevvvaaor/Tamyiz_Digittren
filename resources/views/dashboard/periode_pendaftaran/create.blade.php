...
<div style="">
    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="mb-0">Tambah Periode Pendaftaran</h5>
            <a href="{{ route('periode_pendaftaran.index') }}" class="btn btn-secondary"><i class="fas fa-arrow-left"></i> Kembali</a>
        </div>
        <div class="card-content">
            <div class="card-body">
                <form method="POST" action="{{ route('periode_pendaftaran.store') }}" id="bankForm">
                    @csrf
                    <div class="form-group">
                        <label for="periode">Periode</label>
                        <div class="input-group">
                            <!-- Input asli untuk pengguna -->
                            <input type="number" class="form-control @error('periode') is-invalid @enderror" id="periode" name="periode_display" value="{{ old('periode') }}" placeholder="Masukkan Periode">
                    ...
                    <!-- membuat diskon bisa koma atau float -->
                    <div class="form-group">
                        <label for="diskon">Diskon</label>
                        <input type="text" class="form-control @error('diskon') is-invalid @enderror" id="diskon" name="diskon" value="{{ old('diskon', $periode->diskon ?? '') }}" placeholder="Masukkan Diskon" step="0.01">
                        @error('diskon')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
